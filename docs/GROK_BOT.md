...
- Claim per process with `--author grok-bot/consulting-leads` or `SECOND_BRAIN_IDENTITY=grok-bot/consulting-leads`
- Do **not** use a single shared `knowledge/.identity.json` for a fleet.
- Chat prefix (optional): `Grok Bot: Consulting`
...
**Required:** type ownership. This pack may write: ConsultingLead, EngagementType, DiscoveryCall, Scope, QualificationNote. Refuse everything else.
