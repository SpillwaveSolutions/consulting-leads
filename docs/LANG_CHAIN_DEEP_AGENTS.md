...
agent = create_deep_agent(
    ...
    skills=["./path/to/consulting-leads/skills/"],
)
```
...
SkillsMiddleware(
    backend=backend,
    sources=["/skills/", "/path/to/consulting-leads/skills/"],
)
```

```bash
npx skills add SpillwaveSolutions/consulting-leads --skill '*' --yes
```
...
python3 path/to/second-brain-core/scripts/brain_session.py open \
  --repo "$BRAIN_REPO" \
  --bundle knowledge \
  --actor deep-agents/consulting-leads \
...
```bash
export SECOND_BRAIN_IDENTITY="deep-agents/consulting-leads"
