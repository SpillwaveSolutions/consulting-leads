...
SCRIPT = ROOT / "scripts" / "csl_common.py"
...
def test_write_requires_author():
    with tempfile.TemporaryDirectory() as td:
        ...
        run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "consulting-leads")
        r = run(
            "write",
            "--bundle",
            str(bundle),
...
def test_init_and_write():
    with tempfile.TemporaryDirectory() as td:
        bundle = Path(td) / "knowledge"
        r = run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "consulting-leads")
        assert r.returncode == 0, r.stdout + r.stderr
        r = run(
            "write",
            "--bundle",
        ...
        assert r.returncode == 0, r.stdout + r.stderr
        data = json.loads(r.stdout)
        assert data["author"] == "grok-bot/consulting-leads"
        assert data["event"]
...
def test_isolation_two_sessions_do_not_clobber():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = run("init-bundle", "--bundle", str(knowledge), "--title", "Shared", "--catalogs", "consulting-leads")
        assert r.returncode == 0, r.stdout + r.stderr
        git(repo, "add", ".")
        ...
        assert (Path(sa["bundle"]) / "consulting-leads" / "session-a-note.md").exists()
        assert not (Path(sa["bundle"]) / "consulting-leads" / "session-b-note.md").exists()
        assert (Path(sb["bundle"]) / "consulting-leads" / "session-b-note.md").exists()
        assert not (Path(sb["bundle"]) / "consulting-leads" / "session-a-note.md").exists()
        ...
        assert (knowledge / "consulting-leads" / "session-a-note.md").exists()
        assert (knowledge / "consulting-leads" / "session-b-note.md").exists()


def test_sample_pack_walks():
    root = "Northstar ConsultingLead"
